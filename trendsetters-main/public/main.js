...
// =====================
// SIDEBAR RESIZING (throttled + guards)
// =====================
...
if (sidebar && sidebarDivider) {
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    lastSidebarX = e.clientX;
    if (sidebarRAF) return;
    sidebarRAF = requestAnimationFrame(() => {
      sidebarRAF = null;
      const newWidth = Math.min(Math.max(lastSidebarX, 150), 500);
      sidebar.style.flex = `0 0 ${newWidth}px`;
      if (window.cy) cy.resize();
      if (window.cyTable) cyTable.resize();
      // if (window.cyRecord) cyRecord.resize?.();
    });
  });

  window.addEventListener("mouseup", () => {
    isResizingSidebar = false;
    if (sidebarRAF) { cancelAnimationFrame(sidebarRAF); sidebarRAF = null; }
  });
} else {
  console.warn("Sidebar or sidebar divider not found at init; skipping sidebar resize wiring.");
}


// =====================
// QUERY PANEL RESIZING (throttled + guards)
// =====================
const queryPanel = document.getElementById("query-panel");
const graphArea  = document.getElementById("graph-area");

if (queryPanel && graphArea) {
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    lastQueryY = e.clientY;
    if (queryRAF) return;
    queryRAF = requestAnimationFrame(() => {
      queryRAF = null;
      const parentRect = queryPanel.parentNode.getBoundingClientRect();
      const offset = Math.min(Math.max(lastQueryY - parentRect.top, 100), parentRect.height - 200);
  ...
  window.addEventListener("mouseup", () => {
    ...
    if (queryRAF) { cancelAnimationFrame(queryRAF); queryRAF = null; }
  });
} else {
  console.warn("Query panel or graph area not found; skipping query resize wiring.");
}
