...
export function attachTooltip(cyInstance) {
  ...
  function positionTooltip(originalEvt) {
    const pad = 10;                         // gap from cursor
    const vw = window.innerWidth;
    const vh = window.innerHeight;

    // Use clientX/Y (viewport coords)
    let left = originalEvt.clientX + pad;
    let top  = originalEvt.clientY + pad;

    const rect = tooltip.getBoundingClientRect();

    // If overflowing right, flip to the left of the cursor
    if (left + rect.width > vw) {
      left = originalEvt.clientX - rect.width - pad;
    }
    // If overflowing bottom, flip above the cursor
    if (top + rect.height > vh) {
      top = originalEvt.clientY - rect.height - pad;
    }
  ...
  cyInstance.on("mouseover", "node", (event) => {
    ...
    positionTooltip(event.originalEvent); // New helper used here
  });

  cyInstance.on("mousemove", "node", (event) => {
    positionTooltip(event.originalEvent);
  });
 ...
 // Edge tooltips
  cyInstance.on("mouseover", "edge", (event) => {
    ...
    tooltip.innerHTML = formatTooltip(edge.data());
    tooltip.style.display = "block";
    positionTooltip(event.originalEvent); //  Same helper
  });
  ...
  // Hide tooltip when mouse leaves Cytoscape container
  cyInstance.container().addEventListener("mouseleave", () => {
    tooltip.style.display = "none";
  });

  // Hide tooltip when cursor moves into another container (outside Cytoscape)
  document.addEventListener("mouseover", (e) => {
    if (!cyInstance.container().contains(e.target)) {
      tooltip.style.display = "none";
    }
  }, true);
...
function formatTooltip(data) {
  ...
  if (data.type === "record") {
    return `Table: ${data.table_name || "N/A"}\nColumn: ${data.column_name || "N/A"}\nValue: ${data.value || "N/A"}\nName: ${data.name_col_value || "N/A"}\nDescription: ${data.description || "N/A"}`;
  }
  ...
  `;
    ...
    // Special handling for record edges in record graphs
  if (data.type === "record-edge") {
    return `Direction: ${data.arrow_direction || "N/A"}`;
  }
