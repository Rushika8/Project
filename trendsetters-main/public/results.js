...
export function displayResults(data, tableName) {
  ...
  // ✅ --- STEP 1: Reorder columns (PK/FK first) ---
  const pkFkCols = getPkFkColumns(tableName || ""); // your existing PK/FK helper
  const sampleRow = data[0];
  const allCols = Object.keys(sampleRow);

  // Move PK/FK columns to the left
  const pkfkColumns = allCols.filter(col => pkFkCols.has(col));
  const nonPkfkColumns = allCols.filter(col => !pkFkCols.has(col));
  const orderedColumns = [...pkfkColumns, ...nonPkfkColumns];
  ...
  data.forEach(row => {
    ...
    orderedColumns.forEach(col => {
      ...
      // ✅ Node ID clickable logic
      else if (col === "node_id" && val !== null) {
        const span = document.createElement("span");
        span.textContent = valueText;
...
export function generateFlowReport() {
  if (!window.cyFlow && !window.cyRecord) {
    alert("No flow or record graph available.");
    return;
  }
  ...
  // --- Collect data from Flow graph if exists ---
  if (window.cyFlow) {
  ...
  // --- Collect data from Record graph if exists ---
  if (window.cyRecord) {
    const cyRecord = window.cyRecord;
    const visibleNodes = cyRecord.nodes(':visible');
    const visibleEdges = cyRecord.edges(':visible');

    nodesReport.push(
      ...visibleNodes.map(n => ({
        id: n.data('id'),
        label: n.data('label'),
        type: n.data('type') || 'Record'
      }))
    );
  ...
  content.innerHTML = `
    ...
    <ul>${report.nodes
      .map(
        n => `<li><strong>${n.label}</strong> (${n.type}) — ID: ${n.id}</li>`
      )
      .join("")}</ul>
  ...
  // --- Make draggable (if helper function exists) ---
