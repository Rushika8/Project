...
export function resetRecordGraphUI() {
  const recordSearch = document.getElementById("recordSearch");
  const resetBtn = document.getElementById("resetHighlights");
  const hiddenList = document.getElementById("hidden-node-list");

  if (recordSearch) recordSearch.value = "";
  if (resetBtn) resetBtn.disabled = true;
  if (hiddenList) hiddenList.innerHTML = "";

  if (typeof cyRecord !== "undefined" && cyRecord) {
    cyRecord.elements().removeClass("highlighted");
    cyRecord.elements().removeClass("dimmed");
  }
}
