// sidebar.js
import { cy, showMenu } from "/public/graph.js";
import { fetchTopRows, fetchJoins } from "/public/api.js";
...
export function buildSidebar(tables, data) {
    ...
    function render(filteredTables) {
        ...
        // Right-click (context menu) on a table label → show menu
        list.querySelectorAll(".table-label").forEach(label => {
            label.addEventListener("contextmenu", e => {
                e.preventDefault();
                e.stopPropagation();

                const li = label.closest("li.table-item");
                const table = li.dataset.entity;

                showMenu(e.pageX, e.pageY, [
                    { label: "Generate SELECT * Query", action: () => fetchTopRows(table) },
                    { label: "Generate SELECT with JOINs", action: () => fetchJoins(table) }
                ]);
            });
        });
